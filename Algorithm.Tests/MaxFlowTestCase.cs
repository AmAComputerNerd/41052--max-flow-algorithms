...
{
    public abstract class MaxFlowTestCase
    {
        protected abstract IMaxFlowAlgorithm CreateAlgorithm(FlowGraph graph);
        ...
        public void FindMaxFlow_AfterResetFlow_ReturnsOriginalFlow()
        {
            ...
            graph.AddEdge("S", "A", 10);
            graph.AddEdge("A", "T", 10);
            var algorithm1 = CreateAlgorithm(graph);

            // Act
            var maxFlow1 = algorithm1.FindMaxFlow();
            graph.ResetFlow();
            var algorithm2 = CreateAlgorithm(graph);
