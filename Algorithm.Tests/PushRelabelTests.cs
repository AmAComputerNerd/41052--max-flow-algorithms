...
[TestClass]
public sealed class PushRelabelTests
{
    [TestMethod]
    public void FindMaxFlow_StandardGraph_ReturnsCorrectFlow()
    {
        ...
        var pushRelabel = new PushRelabel(graph);

        // Act
        var maxFlow = pushRelabel.FindMaxFlow();
    ...
    public void FindMaxFlow_AfterResetFlow_ReturnsOriginalFlow()
    {
        ...
        graph.AddEdge("S", "A", 10);
        graph.AddEdge("A", "T", 10);
        var algorithm1 = new PushRelabel(graph);

        // Act
        var maxFlow1 = algorithm1.FindMaxFlow();
        graph.ResetFlow();
        var algorithm2 = new PushRelabel(graph);
        var maxFlow2 = algorithm2.FindMaxFlow();
